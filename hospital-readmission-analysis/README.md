...
This repo tracks code, dashboard, docs, and outputs in git, but **not** the four data CSVs — the raw extract and three pipeline outputs together are ~65MB, which isn't worth carrying in git history for a portfolio repo. You have two ways to get them:

1. **Download the full dataset bundle:** [link to release/Drive — add here]. Drop the files into `data/raw/` and `data/processed/` per the `README.md` placeholders in each folder, and everything (including the Power BI file) works as-is.
2. **Regenerate from scratch:** download the original data from the [UCI source](https://archive.ics.uci.edu/dataset/296/diabetes+130-us+hospitals+for+years+1999-2008), place `diabetic_data.csv` in `data/raw/` (the small `IDS_mapping.csv` lookup is already included), and run `notebooks/cleaning.ipynb` end-to-end — it reproduces `diabetic_data_mapped.csv`, `diabetic_data_cleaned.csv`, and `Final_Diabetic_Data.csv` exactly.

## Repository structure

```
├── data/
│   ├── raw/                        # IDS_mapping.csv included; diabetic_data.csv not tracked (see Data above)
│   └── processed/                  # Pipeline outputs — not tracked, generated by the notebook (see Data above)
